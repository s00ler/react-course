...
export const addComment = (comment) => ({type: ActionTypes.ADD_COMMENT, payload: comment});
...
export const addPromos = (promos) => ({type: ActionTypes.ADD_PROMOS, payload: promos});
...
export const promosFailed = (errmess) => ({type: ActionTypes.PROMOS_FAILED, payload: errmess});
...
export const leadersFailed = (errmess) => ({type: ActionTypes.LEADERS_FAILED, payload: errmess});
...
export const postFeedback = (feedback) => (dispatch) => {
    ...
    return fetch(baseUrl + 'feedback', {
        ...
        let errorMessage = new Error(error.errorMessage);
        throw errorMessage;
    }).then(response => response.json()).then(response => {
        dispatch(addComment(response))
        alert('Thank for yor feedback!\n' + JSON.stringify(response));
    }).catch(error => {
